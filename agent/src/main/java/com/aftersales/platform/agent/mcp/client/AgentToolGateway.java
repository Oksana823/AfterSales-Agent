...
public class AgentToolGateway {
    ...
    public AgentToolGateway(McpToolClient mcpClient, HarnessProperties properties, ToolCallBudgetService budget) { this.mcpClient = mcpClient; this.properties = properties; this.budget = budget; }
    public OrderInfo getLatestOrder(Long runId, Long userId) { return invoke(runId, "getLatestOrder", Map.of("userId", userId), OrderInfo.class); }
    public OrderInfo getOrder(Long runId, Long orderId) { return invoke(runId, "getOrder", Map.of("orderId", orderId), OrderInfo.class); }
    public Boolean isDelayedShipment(Long runId, Long orderId) { return invoke(runId, "isDelayedShipment", Map.of("orderId", orderId), Boolean.class); }
    public OrderInfo cancelOrder(Long runId, Long orderId, String reason) { return invoke(runId, "cancelOrder", Map.of("orderId", orderId, "reason", reason), OrderInfo.class); }
    public List<Product> searchProducts(Long runId, String keyword) { return invoke(runId, "searchProducts", Map.of("keyword", keyword), new TypeReference<List<Product>>() {}); }
    ...
    private <T> T invoke(Long runId, String name, Map<String,Object> args, TypeReference<T> type) { ensureEnabled(); budget.acquire(runId); return mcpClient.call(runId, name, args, type); }
    private void ensureEnabled() { if (!properties.isMcpClientEnabled()) throw new IllegalStateException("Business MCP Client 已关闭"); }
}
