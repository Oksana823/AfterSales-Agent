...
public class ReporterAgent {
    ...
    public ReporterAgent(AiModelService aiModel) { this.aiModel = aiModel; }

    public String afterSalesReply(Long runId, Long ticketId, Long orderId, String productName, String policy) {
        String nl = System.lineSeparator();
        String facts = "工单ID：" + ticketId + nl + "订单ID：" + orderId + nl + "商品：" + productName + nl + "售后政策：" + policy;
    ...
    public String cancelWaiting(Long approvalId, Long orderId) { return "订单" + orderId + "属于敏感取消操作，已创建审批单" + approvalId + "。审批通过前不会真正取消订单。"; }
    public String cancelApproved(Long orderId) { return "审批已通过，订单" + orderId + "已取消，状态变为 CANCELLED。"; }

    public String productAdvice(Long runId, List<Product> products) {
        if (products.isEmpty()) return "没有找到匹配商品，请调整预算或关键词。";
        String nl = System.lineSeparator();
        String facts = products.stream().map(p -> "商品ID=" + p.id() + "，名称=" + p.name() + "，价格=" + p.price() + "，描述=" + p.description()).reduce((a,b) -> a + nl + b).orElse("");
    ...
    private String defaultProductAdvice(List<Product> products) {
        StringBuilder text = new StringBuilder("【系统提示：智能推荐暂时不可用，以下为 Elasticsearch 检索结果，未经过 AI 分析】");
        for (Product p : products) text.append(System.lineSeparator()).append("■ ").append(p.name()).append("  ￥").append(p.price()).append(" | ").append(p.description());
